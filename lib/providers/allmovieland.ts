...
export default async function getAllMovieLandStream(id: string) {
    try {
        ...
        for (const playerUrl of playerUrlCandidates) {
            ...
            for (const path of paths) {
                ...
                console.log(`[AllMovieLand] Trying path: ${targetUrl}`);
                ...
                for (const referer of domainReferers) {
                    try {
                        ...
                        console.log(`[AllMovieLand] Failed path ${targetUrl} with referer ${referer}: ${e.message}`);
                        lastError = e;
                    }
                }
            }
        ...
        return {
            success: false,
            message: lastError ? `AllMovieLand Error: ${lastError.message}` : "AllMovieLand: Media not found on any known paths"
        };
    } catch (error: any) {
        console.error(`[AllMovieLand] Error:`, error.message);
        return {
            success: false,
            message: `AllMovieLand Error: ${error.message}`,
        };
    }
}
